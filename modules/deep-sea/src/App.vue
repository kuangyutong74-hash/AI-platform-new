...
function handleLevelComplete(data) {
  if (data.level === 'LEVEL_1') {
    ...
    void sendEvidence({ module: 'deep_sea', event_type: 'spatial_solution', evidence_level: data.raw_metrics?.is_connected === false ? 'reference' : 'strong', intelligence_candidates: ['spatial', 'logical_mathematical'], behavior_summary: '通过旋转和调整管件完成能源线路布局。', raw_evidence: { completed: true, duration_seconds: data.duration ?? 0, title: '洋流电网重建', pipes_used: data.pipes_used ?? 0, rotate_count: data.raw_metrics?.rotate_count ?? 0, check_attempts: data.raw_metrics?.check_attempts ?? null, connected: data.raw_metrics?.is_connected !== false, renderer_version: data.raw_metrics?.renderer_version ?? 1, grid_rows: data.raw_metrics?.grid_rows ?? 8, grid_cols: data.raw_metrics?.grid_cols ?? 10, pipe_layout: data.raw_metrics?.pipe_layout ?? [], obstacle_layout: data.raw_metrics?.obstacle_layout ?? [], start_cell: data.raw_metrics?.start_cell ?? { row: 0, col: 0 }, end_cell: data.raw_metrics?.end_cell ?? { row: 7, col: 9 } }, context: { activity_id: gameState.studentId, level: 2, idempotency_key: `${gameState.studentId}:level-2` } }).catch(() => {})
    ...
    void sendEvidence({ module: 'deep_sea', event_type: 'mediation_response', evidence_level: (data.harmony_score ?? 0) >= 80 ? 'strong' : 'reference', intelligence_candidates: ['interpersonal', 'linguistic'], behavior_summary: '在角色分歧中识别双方需要，并尝试提出协调方案。', raw_evidence: { completed: true, duration_seconds: data.duration ?? 0, title: '海洋议事厅调解', harmony_band: (data.harmony_score ?? 0) >= 80 ? 'high' : 'developing', harmony_final: data.harmony_score ?? 0, rounds_used: data.raw_metrics?.rounds_used ?? null, supportive_choices: data.raw_metrics?.supportive_choices ?? null, renderer_version: 1, solution_summary: data.raw_metrics?.solution_summary ?? '', sentence_blocks_used: data.raw_metrics?.sentence_blocks_used ?? 0 }, context: { activity_id: gameState.studentId, level: 3, idempotency_key: `${gameState.studentId}:level-3` } }).catch(() => {})

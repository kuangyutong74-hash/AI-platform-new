...
async function goNextLevel() {
  ...
  const logData = {
    ...
    raw_metrics: {
      ...
      grid_cols: COLS,
      renderer_version: 1,
      pipe_layout: flatGrid.value.filter(cell => cell.pipe).map(cell => ({ row: cell.row, col: cell.col, def: cell.pipe.def, rot: cell.pipe.rot, energized: cell.energized })),
      obstacle_layout: flatGrid.value.filter(cell => cell.obstacle).map(cell => ({ row: cell.row, col: cell.col, kind: cell.obstacle.kind })),
      start_cell: start,
      end_cell: end,
      // 📊 行为量化评分系统 —— 无效交互数据

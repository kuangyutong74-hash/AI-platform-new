...
function showAllDone(){
  if(!G.stageResults[G.stage+'-counted']){G.process.completedStages++;G.stageResults[G.stage+'-counted']=true}
  saveProcessRecord();
  if(window.AIBole&&!sessionStorage.getItem('ai-bole-career-evidence-'+G.runId)){const record=processRecord();sessionStorage.setItem('ai-bole-career-evidence-'+G.runId,'1');window.AIBole.emitEvidence({capture_selector:'#wd-main',module:'career',event_type:'workday_process_summary',evidence_level:record.completedStages>=record.stageCount&&record.interactionCount>0?'strong':'reference',intelligence_candidates:['intrapersonal','logical_mathematical'],behavior_summary:'完成职业日常任务，并在过程中根据结果进行尝试和调整。',raw_evidence:{completed:true,work_kind:'career_day',work_content:'完成'+record.career+'的一天：'+record.completedStages+'/'+record.stageCount+'个阶段，主动尝试'+record.interactionCount+'次，调整或重试'+(record.adjustmentCount+record.retryCount)+'次。',duration_seconds:record.durationSeconds,title:record.career+'的一天',career_name:record.career,completed_stages:record.completedStages,stage_count:record.stageCount,adjustment_count:record.adjustmentCount,retry_count:record.retryCount,hint_count:record.hintCount,interaction_count:record.interactionCount},context:{activity_id:G.runId,workday_run_id:G.runId,career_id:G.careerId,idempotency_key:G.runId+':completed'}}).catch(()=>{})}

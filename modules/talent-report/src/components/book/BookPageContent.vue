...
import type { GeneratedReport, ParentAnswerReference } from "../../api/core";
...
const props=defineProps<{spreadIndex:number;side:"left"|"right";talents:Talent[];insights:string[];richNames:string[];fewNames:string[];family:string[];teacher:string[];familyAttributions:ParentAnswerReference[][];teacherAttributions:ParentAnswerReference[][];liveReport?:GeneratedReport;highlightedId?:string}>();
...
<template>
  ...
  <AdviceSpread v-else :side="side" :family="family" :teacher="teacher" :family-attributions="familyAttributions" :teacher-attributions="teacherAttributions" @finish="emit('finish')"/>
</template>

...
import { generateTalentStories, getTalentEligibility } from "../api/core";
import { continentHints, loadStarReflections, loadStarState, mapCopy, resetStarReflections, resetStarState, saveStarReflections, saveStarState, type StarReflection, type StarReflections } from "../data/treasureMap";
...
const reflections=ref<StarReflections>({});
...
onMounted(async()=>{const keys=availableTalents.value.map(t=>t.key);const state=loadStarState(keys);discovered.value=state.discovered;order.value=state.order;reflections.value=loadStarReflections(keys);const response=await getTalentEligibility();eligible.value=response?response.talents.filter(item=>item.eligible&&keys.includes(item.key)).map(item=>item.key):[];if(response){const validDiscovered=discovered.value.filter(key=>eligible.value.includes(key));if(validDiscovered.length!==discovered.value.length){discovered.value=validDiscovered;order.value=order.value.filter(key=>validDiscovered.includes(key));saveStarState({discovered:discovered.value,order:order.value})}}revisit.value=keys.length>0&&discovered.value.length===keys.length;if(response?.talents.some(item=>item.completedModules?.length)){storiesLoading.value=true;storiesFailed.value=false;const result=await generateTalentStories();storiesLoading.value=false;storiesFailed.value=!result;const stories=Object.fromEntries((result?.stories||[]).map(item=>[item.key,item.story]));childStories.value={...childStories.value,...Object.fromEntries(response.talents.filter(item=>keys.includes(item.key)).map(item=>{const moduleId=moduleByTalent[item.key];return [item.key,item.completedModules.includes(moduleId)?stories[item.key]||"":""]}).filter(([,text])=>Boolean(text)))}}});
...
function collectStar(talent:Talent){if(!eligible.value.includes(talent.key)||discovered.value.includes(talent.key))return;discovered.value=[...discovered.value,talent.key];order.value=[...order.value,talent.key];saveStarState({discovered:discovered.value,order:order.value});burstKey.value=talent.key;announcement.value=mapCopy.foundBurst(talent.childName);window.clearTimeout(burstTimer);burstTimer=window.setTimeout(()=>{burstKey.value=undefined},900)}
function closeStory(){selected.value=undefined;if(isComplete.value&&!revisit.value)celebrating.value=true}
function resetMap(){resetStarState();resetStarReflections();discovered.value=[];order.value=[];reflections.value={};selected.value=undefined;celebrating.value=false;revisit.value=false;announcement.value="星星回到藏宝图上啦"}
function saveReflection(key:string,value:StarReflection){reflections.value={...reflections.value,[key]:value};saveStarReflections(reflections.value);announcement.value="你的想法已经收进这颗星星里了"}

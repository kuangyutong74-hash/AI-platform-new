...
import { useBookFlip } from "../composables/useBookFlip";import "../styles/book.css";import "../styles/report-polish.css";import "../styles/serious-report.css";
...
const displayTalents=computed(()=>props.talents.map(talent=>{if(!liveEvents.value.length)return {...talent,evidence:[],moments:[]};const events=liveEvents.value.filter(event=>event.reportDimensions.includes(talent.key));const explanations=liveReport.value?.evidence_explanations||[];return {...talent,evidence:events.map(event=>{const explanation=explanations.find(item=>item.evidence_ref===event.id);return {id:event.id,behavior:explanation?.summary||event.behaviorSummary,source:sourceNames[event.moduleId]||"探索活动",continent:`${talent.continent} · ${sourceNames[event.moduleId]||"探索活动"}`,time:event.occurredAt.replace("T"," ").slice(0,16),level:event.evidenceLevel,raw:"",logTitle:explanation?.title||"报告智能体正在整理",logSummary:explanation?.summary||"报告智能体正在读取这次活动记录…",logDetails:explanation?.details||["生成完成后会显示这次活动中具体发生的过程。"]} as Evidence}),moments:momentsForTalent(liveEvents.value,talent.key).map(moment=>{const explanation=explanations.find(item=>item.evidence_ref===moment.evidenceId);return {...moment,title:explanation?.title||moment.title,caption:explanation?.summary||"报告智能体正在整理这个精彩瞬间…"}})}}));
...
function exportPdf(){document.querySelector<HTMLButtonElement>(".report-footer .primary")?.click()}function setReportFormat(value:"book"|"formal"){reportFormat.value=value;localStorage.setItem("ai-bole-adult-report-format",value)}function onKeydown(event:KeyboardEvent){if(reportFormat.value!=="book")return;if(event.key==="ArrowLeft"){event.preventDefault();opened.value?previous():undefined}else if(event.key==="ArrowRight"){event.preventDefault();opened.value?forward():openBook()}}

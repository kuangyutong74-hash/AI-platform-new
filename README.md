...
- **成长足迹**：区分今天的探索与长期积累，汇总首次/最近完成、活跃天数、跨模块过程证据和高光作品；成长线索只用于持续观察，不作固定标签。
...
- 自动账号固定为 9 位（例如 `S20260001`）。登录、密码重置和学生绑定会兼容 `S2026001`、`S2026-001` 这类漏写一个前导零的常见输入。

本地启动时会幂等创建两组默认测试账号，成人账号已绑定学生并带有两件示例作品：

| 入口 | 账号 | 密码 |
| --- | --- | --- |
| 学生端 | `student_demo` | `demo1234` |
| 老师/家长端 | `adult_demo` | `demo1234` |

设置 `AI_BOLE_SEED_TEST_ACCOUNTS=0` 可关闭默认账号初始化；通过 `AI_BOLE_TEST_ACCOUNT_PASSWORD` 可在首次创建前覆盖默认密码。

## 学生端与老师/家长端

- 学生端默认进入“探索星球”，只保留“探索星球”“我的作品”“天赋藏宝图”。
- “我的作品”收录四个模块的完成成果；学生可从已完成探索的模块反馈中选择来源记录、作品类型、系统整理的名称与内容，再加入作品册。自主收藏的作品单独保存，不参与天赋证据判断。
- “天赋藏宝图”使用天赋报告现有的儿童地图视角；学生端不能切换到成人报告。
- 老师/家长端默认进入当前学生的“天赋报告”，并保留“作品展柜”和“成长足迹”。
- “作品展柜”只读展示学生全部作品；老师/家长可以发表评论，点评会同步显示在学生的“我的作品”中。
- “成长足迹”从成人观察视角分别展示今天完成了什么，以及长期以来的完成次数、活跃天数、覆盖模块和反复出现的过程线索。
...
- “成长足迹”从账号注册日开始，先展示今天与长期以来的双层概览，再按模块展示首次使用、最近使用、累计次数与可用的累计时长；仍包含星路手账、未点亮提示和作品互跳。

...
function createPlanetTextures() {
  const surface = document.createElement("canvas"), ids = document.createElement("canvas"), clouds = document.createElement("canvas"), hover = document.createElement("canvas"), landMask = document.createElement("canvas"), relief = document.createElement("canvas");
  for (const canvas of [surface, ids, clouds, hover, landMask, relief]) { canvas.width = TEXTURE_WIDTH; canvas.height = TEXTURE_HEIGHT; }
  ...
  return { surface: new THREE.CanvasTexture(surface), clouds: new THREE.CanvasTexture(clouds), hover: new THREE.CanvasTexture(hover), reliefTexture: new THREE.CanvasTexture(relief), hoverCtx, landMask, reliefCanvas: relief, islandPaths, idCtx };
}
...
export default function ThreeGlobe() {
  ...
  useEffect(() => {
    ...
    const renderer = new THREE.WebGLRenderer({ antialias:true, alpha:true, powerPreference:"high-performance" }); renderer.setPixelRatio(Math.min(devicePixelRatio,2)); renderer.outputColorSpace=THREE.SRGBColorSpace; renderer.toneMapping=THREE.ACESFilmicToneMapping; renderer.toneMappingExposure=1.04; host.appendChild(renderer.domElement);
    ...
    // 已确认的儿童绘本水彩四大陆贴图。
    const themedSurface=loader.load("/textures/watercolor-exploration-world-v4.png", texture=>{
      ...
      const source=sampleCtx.getImageData(0,0,TEXTURE_WIDTH,TEXTURE_HEIGHT),idImage=textures.idCtx.createImageData(TEXTURE_WIDTH,TEXTURE_HEIGHT),maskCtx=textures.landMask.getContext("2d")!,maskImage=maskCtx.createImageData(TEXTURE_WIDTH,TEXTURE_HEIGHT),reliefCtx=textures.reliefCanvas.getContext("2d")!,reliefImage=reliefCtx.createImageData(TEXTURE_WIDTH,TEXTURE_HEIGHT);
      for(let offset=0;offset<source.data.length;offset+=4){const r=source.data[offset],g=source.data[offset+1],b=source.data[offset+2],luma=r*.2126+g*.7152+b*.0722,oceanLike=g>r*1.12&&b>r*1.08&&Math.abs(g-b)<55,land=!oceanLike&&luma>105;const pixel=offset/4,x=pixel%TEXTURE_WIDTH,y=Math.floor(pixel/TEXTURE_WIDTH);const moduleIndex=y<TEXTURE_HEIGHT/2?(x<TEXTURE_WIDTH/2?1:0):(x<TEXTURE_WIDTH/2?2:3);idImage.data[offset]=land?moduleIndex+1:0;idImage.data[offset+3]=255;const alpha=land?Math.max(0,Math.min(255,(luma-82)*5)):0;maskImage.data[offset]=255;maskImage.data[offset+1]=255;maskImage.data[offset+2]=255;maskImage.data[offset+3]=alpha;const height=land?Math.max(145,Math.min(242,140+(luma-105)*1.15)):18;reliefImage.data[offset]=height;reliefImage.data[offset+1]=height;reliefImage.data[offset+2]=height;reliefImage.data[offset+3]=255;}
      textures.idCtx.putImageData(idImage,0,0);maskCtx.putImageData(maskImage,0,0);reliefCtx.putImageData(reliefImage,0,0);textures.reliefTexture.needsUpdate=true;interactionReady=true;
    }); themedSurface.colorSpace=THREE.SRGBColorSpace; themedSurface.anisotropy=renderer.capabilities.getMaxAnisotropy();
    const sphereGeometry=new THREE.SphereGeometry(2,160,128); const globe=new THREE.Mesh(sphereGeometry,new THREE.MeshStandardMaterial({map:themedSurface,displacementMap:textures.reliefTexture,displacementScale:.085,displacementBias:-.006,roughness:.95,metalness:0,emissive:"#71829a",emissiveIntensity:.07})); globe.rotation.y=-.35; scene.add(globe);
    ...
    const atmosphere=new THREE.Mesh(new THREE.SphereGeometry(2.13,80,80),new THREE.ShaderMaterial({side:THREE.BackSide,transparent:true,blending:THREE.AdditiveBlending,depthWrite:false,uniforms:{glowColor:{value:new THREE.Color("#c8edff")},viewVector:{value:camera.position}},vertexShader:`varying vec3 n;varying vec3 p;void main(){n=normalize(normalMatrix*normal);p=(modelViewMatrix*vec4(position,1.)).xyz;gl_Position=projectionMatrix*vec4(p,1.);}`,fragmentShader:`uniform vec3 glowColor;varying vec3 n;varying vec3 p;void main(){float i=pow(.72-dot(n,normalize(-p)),3.2);gl_FragColor=vec4(glowColor,i*.56);}`})); scene.add(atmosphere);
    scene.add(new THREE.HemisphereLight("#e9f6fb","#8582a5",1.46)); scene.add(new THREE.AmbientLight("#fff3df",.32)); const key=new THREE.DirectionalLight("#fff5df",2.02);key.position.set(-4,3,5);scene.add(key);const rim=new THREE.DirectionalLight("#b9dceb",.68);rim.position.set(4,-1,-4);scene.add(rim);

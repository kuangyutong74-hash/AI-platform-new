...
export default function WorksPage({account,onNavigate}:{account:Account;onNavigate:(view:NavigationView)=>void}){
  ...
  if(selected)return <section className={`works-view works-detail tone-${selected.tone}`}>
    ...
    <main className={`work-reader ${largeText?"is-large":""}`}>
      ...
      <article>
        ...
        <div className="reader-actions"><button className="gold-button" onClick={()=>onNavigate("timeline")}><ExplorerIcon name="compass"/>看看完成足迹</button><button className="plain-button" onClick={()=>setSelected(null)}>再看一枚高光</button></div>
      </article>
    </main>
  </section>;
  ...
  return <section className="works-view works-home">
    ...
    <header className="works-welcome">
      ...
      <div><h1>{data.account.displayName}的高光收藏册</h1><p>每座大陆只留一枚奖章，记录最充分、最完整或最值得回看的一次。</p></div>
    ...
    <main className="storybook-book">
      <div className="book-title"><h2>四枚大陆高光奖章</h2><p>立体贴纸会随着新的完整探索自动更新</p></div>

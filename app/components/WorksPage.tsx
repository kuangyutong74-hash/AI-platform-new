...
type WorkCandidate={id:string;module:WorkModule;occurredAt:string;label:string;titleOptions:string[];descriptionOptions:string[]};
const defaultWorkTitles:Record<WorkModule,string[]>={
  story:["我的故事创作","我最想收藏的故事片段","故事里的新点子"],
  deep_sea:["我的深海基地方案","这次重建的关键调整","深海任务记录"],
  career:["我的职业体验卡","这次任务里的选择","职业体验新发现"],
  chat:["我的表达小记","这次对话的新发现","我想收藏的一段想法"],
};
function uniqueOptions(values:string[],limit:number){return [...new Set(values.map(value=>value.replace(/\s+/g," ").trim()).filter(Boolean))].map(value=>value.length>limit?`${value.slice(0,limit-1)}…`:value);}
...
export default function WorksPage({account,onNavigate,perspective="student"}:{account:Account;onNavigate:(view:NavigationView)=>void;perspective?:"student"|"adult"}){
  ...
  const [manualCandidateId,setManualCandidateId]=useState(""),[manualTitle,setManualTitle]=useState(""),[manualDescription,setManualDescription]=useState("");
  ...
  const workCandidates=useMemo(()=>{
    if(!data||data.timelineIsDemo)return [] as WorkCandidate[];
    return data.growthOverview.sessions.flatMap(session=>{
      if(!session.module||!moduleOrder.includes(session.module))return [];
      const moduleKey=session.module as WorkModule,sourceId=`feedback:${session.id}`;
      if(data.works.some(item=>item.sourceResourceId===sourceId))return [];
      const related=data.works.find(item=>item.sourceSessionId===session.id);
      const titleOptions=uniqueOptions([related?.title||"",...defaultWorkTitles[moduleKey]],60);
      const descriptionOptions=uniqueOptions([...(session.observations||[]),session.caption,related?.summary||"",related?.detail||""],1000);
      return [{id:sourceId,module:moduleKey,occurredAt:session.occurredAt,label:`${formatDate(session.occurredAt)} · ${session.caption}`,titleOptions,descriptionOptions}];
    });
  },[data]);
  const moduleCandidates=workCandidates.filter(item=>item.module===manualModule);
  const selectedCandidate=moduleCandidates.find(item=>item.id===manualCandidateId)||moduleCandidates[0]||null;
  ...
  function applyCandidate(candidate:WorkCandidate|null){setManualCandidateId(candidate?.id||"");setManualTitle(candidate?.titleOptions[0]||"");setManualDescription(candidate?.descriptionOptions[0]||"");}
  function changeManualModule(moduleKey:WorkModule){setManualModule(moduleKey);applyCandidate(workCandidates.find(item=>item.module===moduleKey)||null);}
  function openManualDialog(){const moduleKey=moduleOrder.find(key=>workCandidates.some(item=>item.module===key))||"story";setManualError("");setManualModule(moduleKey);applyCandidate(workCandidates.find(item=>item.module===moduleKey)||null);setManualOpen(true);}
  async function submitManualWork(event:FormEvent<HTMLFormElement>){event.preventDefault();const formElement=event.currentTarget,form=new FormData(formElement);if(!selectedCandidate||!manualTitle||!manualDescription){setManualError("请先从探索反馈中选择作品名称和内容");return;}setManualLoading(true);setManualError("");try{const response=await fetch(`${CORE_URL}/api/explorer/works`,{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({module:manualModule,work_type:form.get("work_type"),title:manualTitle,description:manualDescription,source_id:selectedCandidate.id})});const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(typeof payload.detail==="string"?payload.detail:"作品没有添加成功");formElement.reset();setManualOpen(false);applyCandidate(null);refresh();}catch(cause){setManualError(cause instanceof Error?cause.message:"作品没有添加成功");}finally{setManualLoading(false);}}
  ...
  async function continueAcrossModule(target:WorkModule){if(!selected||relayLoading)return;const platformModule=PLATFORM_MODULES.find(item=>canonicalModuleId(item.id)===target);if(!platformModule){setRelayError("这座大陆暂时没有开放");return;}setRelayError("");setRelayLoading(target);try{await launchPlatformModule(platformModule,{fromModule:selected.module,sourceTitle:selected.title,prompt:relayCopy[selected.module as WorkModule][target]});}catch(cause){setRelayError(cause instanceof Error?cause.message:"探索接力暂时无法开始");setRelayLoading(null);}}
  ...
  return <section className="works-view works-home"><div className="works-sky" aria-hidden="true"/><header className="works-welcome"><button className="back-to-planet" onClick={()=>onNavigate(adult?"report":"planet")}><ExplorerIcon name="compass"/>{adult?"回天赋报告":"回探索星球"}</button><div><h1>{adult?`${data.account.displayName}的作品展柜`:"这些作品来源于四座大陆"}</h1></div>{adult?<button className="listen-action" aria-pressed={speech.activeKey==="works-intro"&&speech.phase==="playing"} onClick={()=>speech.toggle(`这里收藏了${data.account.displayName}的作品。`,"works-intro")}><ExplorerIcon name="headphones"/>{speech.label("works-intro","听一听")}</button>:<button className="add-work-button" onClick={openManualDialog}>＋ 从探索反馈中选作品</button>}</header><div className={`data-notice ${error?"is-error":""}`} role="status"><ExplorerIcon name="spark"/><span>{data.worksNotice}</span>{error&&<button onClick={retry}>再试一次</button>}</div>
    ...
    {!adult&&manualOpen&&<div className="work-dialog-backdrop"><section className="work-dialog" role="dialog" aria-modal="true" aria-labelledby="add-work-title"><header><div><h2 id="add-work-title">从探索反馈中添加作品</h2><p>选择一段真实完成记录，再从系统整理的名称和内容里挑选。</p></div><button type="button" aria-label="关闭" onClick={()=>setManualOpen(false)}>×</button></header><form onSubmit={submitManualWork}><label>来源大陆<select name="module" value={manualModule} onChange={event=>changeManualModule(event.target.value as WorkModule)}>{moduleOrder.map(moduleKey=><option key={moduleKey} value={moduleKey} disabled={!workCandidates.some(item=>item.module===moduleKey)}>{moduleNames[moduleKey]}{workCandidates.some(item=>item.module===moduleKey)?"":"（暂无可选反馈）"}</option>)}</select></label>{selectedCandidate?<><label>选择一次探索反馈<select value={selectedCandidate.id} onChange={event=>applyCandidate(moduleCandidates.find(item=>item.id===event.target.value)||null)}>{moduleCandidates.map(candidate=><option key={candidate.id} value={candidate.id}>{candidate.label}</option>)}</select></label><label>作品类型<select name="work_type" key={manualModule} defaultValue={workTypeOptions[manualModule][0].value}>{workTypeOptions[manualModule].map(option=><option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label>作品名称<select name="title" value={manualTitle} onChange={event=>setManualTitle(event.target.value)}>{selectedCandidate.titleOptions.map(option=><option key={option} value={option}>{option}</option>)}</select></label><label>作品内容<select name="description" value={manualDescription} onChange={event=>setManualDescription(event.target.value)}>{selectedCandidate.descriptionOptions.map(option=><option key={option} value={option}>{option}</option>)}</select></label><div className="work-option-preview"><ExplorerIcon name="spark"/><p><b>{manualTitle}</b><span>{manualDescription}</span></p></div></>:<div className="work-option-empty"><ExplorerIcon name="compass" size={30}/><p>还没有可以选择的完成反馈。先去这座大陆完成一次探索，回来就能直接加入作品册。</p></div>}{manualError&&<p className="login-error" role="alert">{manualError}</p>}<div><button type="button" onClick={()=>setManualOpen(false)}>取消</button><button className="save-work-button" disabled={manualLoading||!selectedCandidate}>{manualLoading?"添加中…":"添加到作品册"}</button></div></form></section></div>}
  </section>;
}

...
import PlatformMusicPlayer from "./components/PlatformMusicPlayer";
...
type Account={id:string;username:string;display_name:string;age:number;created_at?:string;role:Role;avatar_id?:string|null};
...
export default function Home(){
  ...
  if(session.account.role==="student"&&!session.account.avatar_id)return <IdentitySetup account={session.account} onSaved={setSession} onLogout={logout}/>;
...
const STUDENT_AVATARS=Array.from({length:6},(_,index)=>({id:`student-${index+1}`,src:`http://localhost:3000/assets/avatars/student/student-${index+1}.png`}));
function IdentitySetup({account,onSaved,onLogout,onCancel}:{account:Account;onSaved:(session:Session)=>void;onLogout?:()=>void;onCancel?:()=>void}){
  const [nickname,setNickname]=useState(account.display_name),[avatarId,setAvatarId]=useState(account.avatar_id||"student-1"),[loading,setLoading]=useState(false),[error,setError]=useState("");
  async function save(event:FormEvent){event.preventDefault();setLoading(true);setError("");try{const response=await fetch(`${CORE_URL}/api/account/profile`,{method:"PATCH",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({display_name:nickname,avatar_id:avatarId})});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(apiErrorMessage(data,"身份没有保存成功"));onSaved(normalizeSession(data));}catch(cause){setError(cause instanceof Error?cause.message:"暂时无法保存");}finally{setLoading(false)}}
  const selectedAvatar=STUDENT_AVATARS.find(item=>item.id===avatarId)??STUDENT_AVATARS[0];
  return <main className="identity-page"><section className="identity-panel" role={onCancel?"dialog":undefined} aria-modal={onCancel||undefined} aria-labelledby="identity-title">{onCancel&&<button type="button" className="identity-close" aria-label="关闭形象设置" onClick={onCancel}>×</button>}<div className="identity-copy"><div className="identity-stage" aria-hidden="true"><span className="identity-stage-star">✦</span><img src={selectedAvatar.src} alt=""/><i/></div><p>你的星球角色</p><h1 id="identity-title">{onCancel?"换一个新形象":"选出故事里的你"}</h1><span>这个形象和昵称会陪你进入聊天观察、故事共创和每一次探索。</span></div><form onSubmit={save}><label className="identity-name">大家怎么称呼你？<input value={nickname} onChange={event=>setNickname(event.target.value)} maxLength={30} required autoFocus/></label><fieldset><legend>选择一个角色</legend><div className="identity-avatar-grid">{STUDENT_AVATARS.map((avatar,index)=><button key={avatar.id} type="button" className={avatarId===avatar.id?"selected":""} aria-pressed={avatarId===avatar.id} onClick={()=>setAvatarId(avatar.id)}><img src={avatar.src} alt={`形象 ${index+1}`}/><span>{avatarId===avatar.id?"就是我":"选择"}</span></button>)}</div></fieldset>{error&&<p className="login-error" role="alert">{error}</p>}<button className="primary-button" disabled={loading}>{loading?"正在保存…":onCancel?"保存我的新形象":"带着这个形象出发"}<span>→</span></button>{onCancel?<button type="button" className="text-button" onClick={onCancel}>暂不修改</button>:<button type="button" className="text-button" onClick={onLogout}>退出登录</button>}</form></section></main>
}
...
function AuthPage({onAuthenticated}:{onAuthenticated:(session:Session)=>void}){
  ...
  return <main className="login-page"><div className="login-stars" aria-hidden="true"><i/><i/><i/><i/><i/></div><aside className="login-world" aria-hidden="true"><div className="login-world-orbit"/><img className="login-world-planet" src="/assets/watercolor-brand-planet-v1.png" alt=""/><img className="login-world-friend friend-one" src="http://localhost:3000/assets/avatars/student/student-2.png" alt=""/><img className="login-world-friend friend-two" src="http://localhost:3000/assets/avatars/student/student-5.png" alt=""/><div className="login-world-copy"><strong><span>每个孩子，</span><span>都是一颗</span><span>等待被看见的星星</span></strong><p>从一次聊天、一个故事，到属于自己的天赋星图。</p></div></aside><section className="login-panel" aria-labelledby="auth-title">
    <div className="mini-planet" aria-hidden="true"><span/><b>✦</b></div><p className="login-brand">AI 伯乐</p><h1 id="auth-title">{mode==="reset"?"重新找回探索钥匙":mode==="register"?"创建你的探索账号":"欢迎回到探索星球"}</h1><p className="login-welcome">{mode==="login"?"带上好奇心，我们继续出发。":mode==="register"?"从今天起，慢慢发现独一无二的你。":"设置新密码后，就能继续你的旅程。"}</p>
...
function Header({session,view,onNavigate,onLogout,onSelectStudent,onManageStudents,onEditIdentity}:{session:Session;view:View;onNavigate:(view:View)=>void;onLogout:()=>void;onSelectStudent:(id:string)=>void;onManageStudents:()=>void;onEditIdentity:()=>void}){const adult=session.account.role==="adult",nav:[View,string][]=adult?(session.selected_student?[["report","天赋报告"],["showcase","作品展柜"],["timeline","成长足迹"]]:[]):[["planet","探索星球"],["works","我的作品"],["treasure","天赋藏宝图"]];const identity=adult?session.selected_student:session.account,canEditIdentity=Boolean(identity),avatar=identity?.avatar_id?`http://localhost:3000/assets/avatars/student/${identity.avatar_id}.png`:null;return <header className={`topbar ${view!=="planet"?"personal-topbar":""} view-${view}`}><button className="brand" onClick={()=>onNavigate(adult?"report":"planet")}><span className="brand-mark"><img src="/assets/watercolor-brand-planet-v1.png" alt=""/></span><span className="brand-copy"><b>AI 伯乐</b><small><i/>{adult?"家长 / 老师观察站":"儿童天赋探索星球"}</small></span></button><nav>{nav.map(([id,label])=><button key={id} className={view===id?"active":""} onClick={()=>onNavigate(id)}>{label}</button>)}</nav><div className="topbar-actions">{adult&&session.students.length>0&&<label className="student-switcher"><span>正在查看</span><select value={session.selected_student?.id??""} onChange={e=>onSelectStudent(e.target.value)}>{session.students.map(student=><option key={student.id} value={student.id}>{student.display_name} · {student.username.toUpperCase()}</option>)}</select></label>}{adult&&<button className="manage-students-button" onClick={onManageStudents}>管理学生</button>}<details className="profile-menu"><summary aria-label={canEditIdentity?`打开账号菜单，当前形象是${identity?.display_name}`:"打开账号菜单"}>{avatar?<img className="profile-avatar-image" src={avatar} alt=""/>:<b>{session.account.display_name.slice(0,2)}</b>}<span>{session.account.display_name}</span></summary><div><strong>{identity?.display_name||session.account.display_name}</strong><small>{adult?"当前孩子的形象":`@${session.account.username.toUpperCase()}`}</small>{canEditIdentity&&<button onClick={onEditIdentity}>{adult?"修改当前孩子形象":"修改昵称和形象"}</button>}{adult&&<button onClick={onManageStudents}>管理学生</button>}<button onClick={onLogout}>退出登录</button></div></details></div></header>}

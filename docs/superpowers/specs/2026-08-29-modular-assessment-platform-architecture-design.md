...
> 实施更新（2026-08-30）：当前开发版本已完成 V1-only Contract 收口。V0 `evidence_events`、旧接口、浏览器 bridge、双写与历史回填不再是运行时能力；本地测试数据须由 `modules/platform-core/reset_dev_data.py --confirm` 重建。本文中描述的迁移期兼容策略仅保留为历史设计决策，不应作为当前实现说明。
